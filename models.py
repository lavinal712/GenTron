...
class TextEmbedder(nn.Module):
    def __init__(self, embedding_dim, hidden_size, dropout_prob, act_layer=lambda: nn.GELU(approximate='tanh'), token_num=77):
        super().__init__()
        self.y_proj = Mlp(in_features=embedding_dim, hidden_features=hidden_size, out_features=hidden_size, act_layer=act_layer, drop=0)
        self.register_buffer("y_embedding", nn.Parameter(torch.randn(token_num, embedding_dim) / embedding_dim ** 0.5))
        self.dropout_prob = dropout_prob

    def token_drop(self, text, force_drop_ids=None):
        if force_drop_ids is None:
            drop_ids = torch.rand(text.shape[0], device=text.device) < self.dropout_prob
        else:
            drop_ids = force_drop_ids == 1
...
def GenTronT2I_S_2(**kwargs):
    return GenTronT2I(depth=12, hidden_size=384, patch_size=2, num_heads=6, use_cross_attention=True, **kwargs)

def GenTronT2I_B_2(**kwargs):
    return GenTronT2I(depth=12, hidden_size=768, patch_size=2, num_heads=12, use_cross_attention=True, **kwargs)

def GenTronT2I_L_2(**kwargs):
    return GenTronT2I(depth=24, hidden_size=1024, patch_size=2, num_heads=16, use_cross_attention=True, **kwargs)
...
GenTron_models = {
    ...
    'GenTron-T2I-B/2': GenTronT2I_B_2,
